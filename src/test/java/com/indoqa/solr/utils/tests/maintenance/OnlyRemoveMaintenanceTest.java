...
public class OnlyRemoveMaintenanceTest extends AbstractMaintenanceTest {
    ...
    public static void setupSchema() {
        try {
            CHANGED_SCHEMA = new SolrSchema(
                INITIAL_COLLECTION,
                "src/test/resources/solr/maintenance/only_remove_changed/conf/schema.xml");
    ...
    public void test() throws InitializationFailedException {
        ...
        ClasspathSolrCoreMaintainer classpathSolrCoreMaintainer = new ClasspathSolrCoreMaintainerBuilder()
            .addConfiguration(configOf(getRunningSolr(), CHANGED_SCHEMA, ALLOW_REMOVING_FIELDS))
            .build();
        classpathSolrCoreMaintainer.initialize();

        schema = getSchema();
        assertEquals(0, schema.getCopyFields().size());
        assertEquals(1, schema.getFields().size());
        assertEquals(0, schema.getDynamicFields().size());
        assertEquals(1, schema.getFieldTypes().size());

    }

    protected SolrClient getRunningSolr() {
        return getSolrInfrastructure().getOnlyRemoveSolrClient();
    }
