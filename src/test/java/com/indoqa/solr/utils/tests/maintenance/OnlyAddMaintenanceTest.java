...
public class OnlyAddMaintenanceTest extends AbstractMaintenanceTest {
    ...
    public static void setupSchema() {
        try {
            CHANGED_SCHEMA = new SolrSchema(
                INITIAL_COLLECTION,
                "src/test/resources/solr/maintenance/only_add_changed/conf/schema.xml");
    ...
    public void test() throws InitializationFailedException {
        ...
        ClasspathSolrCoreMaintainer classpathSolrCoreMaintainer = new ClasspathSolrCoreMaintainerBuilder()
            .addConfiguration(configOf(getRunningSolr(), CHANGED_SCHEMA, ALLOW_ADDING_FIELDS))
            .build();
    ...
    protected SolrClient getRunningSolr() {
        return getSolrInfrastructure().getOnlyAddSolrClient();
    }
